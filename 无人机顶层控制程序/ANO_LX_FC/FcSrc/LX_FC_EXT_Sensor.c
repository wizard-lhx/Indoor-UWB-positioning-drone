...
static inline void General_Distance_Data_Handle()
{
	...
	Position_Z = ext_sens.gen_dis.st_data.distance_cm;
}

//将接收到的UWB位置信息触发发送（发给IMU，方便上位机读取）
void General_Position_Data_Handle()
{
	UWB_DT(); //处理串口2接收回来的UWB原始数据转为位置信息
	fc_pos.st_data.laser_z = Position_Z;
	KeyBoard_DT();//处理串口3接收回来的键盘数据
	dt.fun[0xf1].WTS = 1;//将要发送fc_pos数据。
	dt.fun[0xf2].WTS = 1;//将要发送key_pos数据。
}
