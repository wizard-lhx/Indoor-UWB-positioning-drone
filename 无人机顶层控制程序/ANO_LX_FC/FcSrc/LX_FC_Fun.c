...
//升高（距离cm，速度cmps）
u8 Vertical_Up(u16 distance_cm, u16 velocity_cmps)
{
	//
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
...
//降低（距离cm，速度cmps）
u8 Vertical_Down(u16 distance_cm, u16 velocity_cmps)
{
	//
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
...
//左旋（角度0-359 deg，速度：5-90 deg/s）
u8 Rotate_Left(u16 rotate_deg, u16 velocity_degps) 
{
	//
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		...
		dt.cmd_send.CMD[1] = 0X07;
		//
		dt.cmd_send.CMD[2] = BYTE0(rotate_deg);
		dt.cmd_send.CMD[3] = BYTE1(rotate_deg);
		dt.cmd_send.CMD[4] = BYTE0(velocity_degps);
		dt.cmd_send.CMD[5] = BYTE1(velocity_degps);
		//
		CMD_Send(0xff, &dt.cmd_send);
		return 1;
	}
...
//右旋（角度0-359 deg，速度：5-90 deg/s）
u8 Rotate_Right(u16 distance_deg, u16 velocity_degps)
{
	//
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
