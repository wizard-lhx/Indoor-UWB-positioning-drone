...
u16   Flash_structure_Mode;     	//0:为测距模式 1:二维模式 3：二维自动模式
u16   Flash_Ranging_Mode; //测距模式
...
u16   Flash_MAJOR_BS_X_Y_Z[3];    //主基站的位置，      X,Y,Z各两字节
u16   Flash_BS_EN_X_Y_Z[7][4];    //7次基站的位置，    使能标志：0为关  1为开   X,Y,Z各两字节
u16 	Flash_TAG_NUM;					//测量标签ID
u8 	  Flash_TAG_BUF[100];      //标签ID列表

u16   Flash_FLAG;	 								//标志位
u16   FLASH_Channel;              //空中信道
u16   FLASH_Data_rat;             //空中传输速率     
u16   FLASH_RX_ANT_DLY;           //接收延时
u16   FLASH_KALMAN_Q;       	    //卡尔曼滤波-Q
u16   FLASH_KALMAN_R;					    //卡尔曼滤波-R

/******************************************************************************
														 计算使用变量
*******************************************************************************/
u16   Calculate_EN;       			//测量使能
u16		Calculate_FLAG;						//测量状态标志
u16		Calculate_TAG_ID;						//测量执行的标签ID
u16		Calculate_TAG_X;					//测量标签的X轴
u16		Calculate_TAG_Y;					//测量标签的Y轴
u16		Calculate_TAG_Z;     			//测量标签的Z轴
u16		Calculate_Station_TAG[8]; //测量标签的距离
u16   Calculate_Last_Dist[100];      //回传距离使用的缓存，用于次基站和主基站模式测得的标签距离下一次回传给标签
u16   Calculate_Last_XYZ[100][3];       //回传坐标使用的缓存，用于次基站和主基站模式测得的标签距离下一次回传给标签
u16   Calculate_Receive_MODE=0;      //标签接收到的距离变量
u16   Calculate_Receive_Dist=0;      //标签接收到的距离变量
u16   Calculate_Receive_XYZ[3];      //标签接收到的距离变量
u16   Calculate_Receive_FLAG=0;      //标签接收到的信息标志位
u16   Calculate_TAG_FLAG=0;      //定位模式中，循环标签ID的标志位
u16   Calculate_KEY_NUM_FLAG=0;      //按键扫描过程的计次标志位
/******************************************************************************
														系统流程循环标志位
*******************************************************************************/
u8 SYS_Calculate_ACTIVE_FLAG=0;   				//系统循环标志位主动测距函数
u8 SYS_Calculate_PASSIVE_FLAG=0;   				//系统循环标志位被动测距函数
u8 SYS_BS_FLAG=0;    				//系统循环标志位-次基站
u8 SYS_MAJOR_BS_FLAG=0;     //系统循环标志位-主基站
u8 SYS_BS_TAG_FLAG=0;       //次基站收到需要测距标签的ID
u8 SYS_BS_MESSAGE_FLAG=0;       //主基站联系次基站标志位
u16 SYS_BS_MESSAGE_Timer_FLAG=0;       //主基站等待次基站回复允许错误数据次数缓存区

/******************************************************************************
														系统流程自定义
*******************************************************************************/

u32 MODBUS_BaudRate[10]={4800,9600,14400,19200,38400,56000,57600,115200,128000,256000};    //波特率列表
u32 AIR_Chan[6]={1,2,3,4,5,7};    //空中信道
int16_t SYS_dis_buf_t[7];  	//主基站获取到的距离数据缓存区
u16 TAG_ID=0;   							//测距标签ID
u16 ERROR_FLAG;  						//测距错误计算次数标志位，达到一定次数跳出
uint16_t LED_FLAG=0;     		//系统指示灯记录标志位
uint16_t KEY_FLAG=0;     		//系统按键记录标志位
unsigned char USART_STR_BUF[300];   //调试用的字符串输出缓存
u16 DWM1000_SET_DATA[3];     //DWM1000监测配置参数是否被改变的缓存数组
/******************************************************************************
														DWM1000测距计算变量
*******************************************************************************/
...
#define RX_BUF_LEN 32               //DWM1000接收数据包长度
...
void OLED_display_data(int16 dist,int16 X,int16 Y,u16 MODE);  //检测数据显示函数

/******************************************************************************
												    基站呼叫标签并进行测距
*******************************************************************************/
int32_t DW1000send(uint8_t A_ID,uint8_t B_ID,uint8_t MODE) //主动模式
{
			  ...
			  uint32 i;
				if(SYS_Calculate_ACTIVE_FLAG==0)
				{
						...
						Send_get_dist_msg[4]=0;    //初始化
						if(MODE==1)
						{
							Send_get_dist_msg[4]=1;     //回传测距信息
							Send_get_dist_msg[5]=Calculate_Last_Dist[B_ID]/256;		//赋予上一次的距离信息
							Send_get_dist_msg[6]=Calculate_Last_Dist[B_ID]%256;		//赋予上一次的距离信息
						}
            else if(MODE==2)		
						{
							Send_get_dist_msg[4]=2;     //回传测距信息
							Send_get_dist_msg[7]=Calculate_Last_XYZ[B_ID][0]/256;		//赋予上一次的坐标信息
							Send_get_dist_msg[8]=Calculate_Last_XYZ[B_ID][0]%256;		//赋予上一次的坐标信息				
							Send_get_dist_msg[9]=Calculate_Last_XYZ[B_ID][1]/256;		//赋予上一次的坐标信息
							Send_get_dist_msg[10]=Calculate_Last_XYZ[B_ID][1]%256;		//赋予上一次的坐标信息										
						}							
						...
						dwt_setrxtimeout(60000);						//设置接收超时时间
	...
	//				printf("%d\n",SYS_Calculate_ACTIVE_FLAG); 
        ...
        if(SYS_Calculate_ACTIVE_FLAG==3)
            ...
            dwt_readrxdata(rx_buffer, frame_len, 0);   //读取接收数据
						...
						if ((rx_buffer[3]==0xBC)&&((rx_buffer[0]==B_ID)&&(rx_buffer[1]==A_ID)))//判断接收到的数据是否是response数据
            {  
			...
			//		printf("%d\n",SYS_Calculate_ACTIVE_FLAG); 
                ...
                dwt_starttx(DWT_START_TX_IMMEDIATE| DWT_RESPONSE_EXPECTED);//设定为延迟发送
					...
					if(SYS_Calculate_ACTIVE_FLAG==7)
					{
							...
							frame_len = dwt_read32bitreg(RX_FINFO_ID) & RX_FINFO_RXFLEN_MASK;	//获得接收到的数据长度
							dwt_readrxdata(rx_buffer, frame_len, 0);   //读取接收数据						
							if ((rx_buffer[3]==0xDE)&&((rx_buffer[0]==B_ID)&&(rx_buffer[1]==A_ID)))//判断接收到的数据是否是response数据
							{
	      ...
	      if(SYS_Calculate_PASSIVE_FLAG==0)  //打开接收
				...
				if(SYS_Calculate_PASSIVE_FLAG==3)   //判断是否为有效数据包
				{
					...
					if(SYS_Calculate_PASSIVE_FLAG==4)  //发送数据后打开接收
					{
							...
							if(rx_buffer[0]==0xFF) //判断是否为主基站的消息
							{
			 ...
			 if(SYS_BS_MESSAGE_FLAG==3)
			 {
			  ...
			  if(SYS_BS_MESSAGE_FLAG==4)
				{	
								...
								frame_len = dwt_read32bitreg(RX_FINFO_ID) & RX_FINFO_RXFLEN_MASK;	//获得接收到的数据长度
								dwt_readrxdata(rx_buffer, frame_len, 0);   //读取接收数据
								if ((rx_buffer[3])==0xFF&&((rx_buffer[0]==B_ID)&&(rx_buffer[1]==A_ID)))//判断接收到的数据是否是response数据
								{     
								...
								else 
								{
									...
									//SYS_BS_MESSAGE_FLAG=0;//不初始化 因为等待回信过程，有测距程序会收到无效数据包											
										...
										{
											ERROR_FLAG=0;//感受到次基站的通讯记录，证明基站存在，错误标志清理
											  SYS_BS_MESSAGE_FLAG=1;	//如果错误就重新接收
												SYS_BS_MESSAGE_Timer_FLAG++;
										}
										return 0;
...
uint8_t DW1000rec_dist_msg(uint8_t B_ID) //次基站等待信号 
{
		...
		if(SYS_BS_FLAG==0)
		{
        dwt_setrxtimeout(65535);//设定接收超时时间，0位没有超时时间
        dwt_rxenable(0);//打开接收
				SYS_BS_FLAG=1;
		}
		if(SYS_BS_FLAG==1)
...
void MODE_TAG(void)
{
      ...
      if(Calculate_Receive_FLAG>6)
			{
				OLED_display_data(Calculate_Receive_Dist,Calculate_Receive_XYZ[0],Calculate_Receive_XYZ[1],Calculate_Receive_MODE);				//屏幕显示会卡住主流程通讯		启动屏幕显示会降低测距速率
				Calculate_Receive_FLAG=0;
			}				
			if(dis_buf==2)  //串口输出
			{
				u16 len=0;
				if(Calculate_Receive_MODE==1)//测距模式
				{
					char a[5]="DIST=";	
					char b[5];		  				
					char c[8]="cm    ";		
				...
				if(Calculate_Receive_MODE==2)//定位模式
				{
					char a[2]="X=";	
					char b[5];		  				
					char c[8]="cm ";					
...
void MODE_SUB_ANCHOR(void)
{
								...
								if(SYS_BS_FLAG==4)
								{
									...
									if(ERROR_FLAG>1500)   //测距发生错误
									{				
											if(SYS_BS_FLAG==4) DW1000send_dist_msg_last(((Flash_Device_ID>>8)&0xFF)+248,255,0);	//如果是定位模式，虽然测距失败，也返回信息
										  SYS_BS_FLAG=0;	
										  SYS_Calculate_ACTIVE_FLAG=0;
										  ERROR_FLAG=0;        //错误标志归0
									}						
...
void MODE_MAJOR_ANCHOR(void)
{
							...
							{
								int8_t o=0;
								if((SYS_MAJOR_BS_FLAG!=0)&&(Flash_TAG_BUF[0] != TAG_ID))//检测过程中标签ID被修改就取消检测重新开始  防止检测过程被修改
									{
											SYS_MAJOR_BS_FLAG=0;							
									}
									...
									{										   		
											SYS_dis_buf_t[0]=DW1000send(255,TAG_ID,1);		//填入寄存器MODBUS														
									...
									{			
										if((Calculate_FLAG>>0)&0x01) //测距成功才滤波刷新，测距失败保留上一次的数据	
										{											
												dist[0] = LP(SYS_dis_buf_t[0],8*TAG_ID);//LP 为低通滤波器，让数据更稳定	
										}
										Calculate_Last_Dist[TAG_ID]=dist[0];//赋予回传标签变量距离值										
									  OLED_display_data(Calculate_Last_Dist[TAG_ID],0,0,1);  //OLED显示距离
										...
										Calculate_Station_TAG[0]=dist[0];//赋予A基站测距值
											...
											if(Calculate_TAG_FLAG<(Flash_TAG_NUM-1))  //ID列表扫描 ，ID变更
											{
												Calculate_TAG_FLAG++;
											}
											else
							...
							if(Flash_structure_Mode==1)//二维定位模式
							 {									
									int8_t o=0,i=0;								  
								  if((SYS_MAJOR_BS_FLAG!=0)&&(Flash_TAG_BUF[Calculate_TAG_FLAG]!= TAG_ID))//检测过程中标签ID被修改就取消检测重新开始  防止检测过程被修改
									{
											SYS_MAJOR_BS_FLAG=0;													
									}
									/*****************************************
									...
									*****************************************/
									if(SYS_MAJOR_BS_FLAG==1) //主基站与标签测距
									{
											SYS_dis_buf_t[0]=DW1000send(255,TAG_ID,2);		//填入寄存器MODBUS																		
											if(SYS_dis_buf_t[0]!=0)
											{
												  Calculate_FLAG|=0x01<<0;		//成功测距赋予标志	
													SYS_MAJOR_BS_FLAG=2;												
						      ...
						      if(SYS_MAJOR_BS_FLAG==9)
									{
										  ...
										  u8 cla_flag=0;
											...
											for(i=0;i<8;i++)
											{					
													if((Calculate_FLAG>>i)&0x01) //测距成功才滤波刷新，测距失败保留上一次的数据
														dist[i]=LP(SYS_dis_buf_t[i],i+8*TAG_ID);//LP 为低通滤波器，让数据更稳定	
											}
										 ...
										 if(cla_flag!=0) //状态显示计算错误
										  {
												  ...
												  Calculate_FLAG&=~(0x01<<8);		//计算失败赋予标志	   
													Calculate_TAG_X=0; //赋予到寄存器
													Calculate_TAG_Y=0; //赋予到寄存器
													Calculate_TAG_Z=0; //赋予到寄存器		
												  Calculate_Last_XYZ[TAG_ID][0]=0;//回传标签坐标值
												  Calculate_Last_XYZ[TAG_ID][1]=0;//回传标签坐标值
												  Calculate_Last_XYZ[TAG_ID][2]=0;//回传标签坐标值
												  Calculate_Station_TAG[0]=dist[0];												
													for(o=0;o<7;o++)   //赋予到寄存器  坐标计算失败，各个基站距离值照常输出
													{
											...
											{
												 Calculate_FLAG|=0x01<<8;		//计算成功赋予标志	                  
												clua_x_y[0] = KalmanFilter(clua_x_y[0],FLASH_KALMAN_Q,FLASH_KALMAN_R,0+2*TAG_ID);    //卡尔曼滤波
												 clua_x_y[1] = KalmanFilter(clua_x_y[1],FLASH_KALMAN_Q,FLASH_KALMAN_R,1+2*TAG_ID);		//卡尔曼滤波
												 Calculate_TAG_X=(int)(clua_x_y[0]); //赋予到寄存器
												 Calculate_TAG_Y=(int)(clua_x_y[1]); //赋予到寄存器
												 Calculate_TAG_Z=0; //赋予到寄存器	
												 Calculate_Last_XYZ[TAG_ID][0]=Calculate_TAG_X;//回传标签坐标值
												 Calculate_Last_XYZ[TAG_ID][1]=Calculate_TAG_Y;//回传标签坐标值
												 Calculate_Last_XYZ[TAG_ID][2]=Calculate_TAG_Z;//回传标签坐标值
												/******************************///赋予值
												Calculate_Station_TAG[0]=dist[0];												
												 for(o=0;o<7;o++)   //赋予到寄存器
												{
													if(Flash_BS_EN_X_Y_Z[o][0]==1) Calculate_Station_TAG[o+1]=dist[o+1];								
													else Calculate_Station_TAG[o+1]=0;							
													}															
												
													OLED_display_data(0, Calculate_Last_XYZ[TAG_ID][0],Calculate_Last_XYZ[TAG_ID][1],2);				//屏幕显示会卡住主流程通讯					
											}
                      ...
                      if(Calculate_TAG_FLAG<(Flash_TAG_NUM-1))//ID列表扫描 ，ID变更
									...
									*/
									if((ERROR_FLAG>1500)&&(SYS_MAJOR_BS_FLAG!=0))   //测距发生错误,并且已启动，错误计数不能调太小，否则干扰次基站测距
									{					
										SYS_MAJOR_BS_FLAG++;
										SYS_Calculate_ACTIVE_FLAG=0;//系统循环标志位主动测距标志位
...
void KEY_SCAN(void)
{
   if(Read_KEY_MODE()==0&&KEY_FLAG==1)//MODE按键按下
	 {
		 Flash_Device_Mode++;
        ...
        Calculate_KEY_NUM_FLAG++;
		 		switch(Flash_Device_Mode)
				{
						...
						{			
							...
							if(KEY_FLAG==1) ID++;  //非重复按键
							if(Calculate_KEY_NUM_FLAG>10)//检测为长按状态
							{
								ID=0;
							}
							if(ID>99) ID=0;
	 ...
	 if(Read_KEY_MODE()==1&&Read_KEY_SET()==1)//按键没被按下就标志位归零
	 {
		 KEY_FLAG=1;
...
void DWM1000_init()
{
    ...
    dwt_setrxantennadelay(FLASH_RX_ANT_DLY);		//设置接收天线延迟
