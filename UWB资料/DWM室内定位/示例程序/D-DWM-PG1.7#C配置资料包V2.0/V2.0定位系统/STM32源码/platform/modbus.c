...
void MODBUS_datain() 
{
	...
	modbus_reg[5]=(FLASH_Channel<<8&0xFF00)|FLASH_Data_rat;//空中信道  空中传输速率
  modbus_reg[6]=FLASH_KALMAN_Q;       	    //卡尔曼滤波-Q
	modbus_reg[7]=FLASH_KALMAN_R;       	    //卡尔曼滤波-R
  modbus_reg[8]=FLASH_RX_ANT_DLY;           //接收延时
...
void MODBUS_dataout() 
{
	  ...
	  if(((modbus_reg[2]>>8)&0xFF)<=1)Flash_Ranging_Mode=(modbus_reg[2]>>8)&0xFF; //测距方式
	  ...
	  if(((modbus_reg[5]>>8)&0xFF)<=5) FLASH_Channel=(modbus_reg[5]>>8)&0xFF;          //空中信道 
	  if((modbus_reg[5]&0xFF)<=5) FLASH_Data_rat=modbus_reg[5]&0xFF;                   //空中传输速率

     FLASH_KALMAN_Q=modbus_reg[6];       	    //卡尔曼滤波-Q
	   FLASH_KALMAN_R=modbus_reg[7];       	    //卡尔曼滤波-R
     FLASH_RX_ANT_DLY=modbus_reg[8];           //接收延时
